...
2. No systems are deployed into Ravenna environments without approval of the Ravenna CTO.
3. All changes to production systems, network devices, and firewalls are approved by the Ravenna CTO before they are implemented to assure they comply with business and security requirements.
...
6. Tooling to generate an up-to-date inventory of systems, including corresponding architecture diagrams for related products and services, is hosted on GitLab.
   ...
   * These scripts are used to generate the diagrams and asset lists required by the Risk Assessment phase of Ravenna's Risk Management procedures ([§4.3.1](#4.3-risk-management-procedures)).
...
10. Ravenna utilizes development and staging environments that mirror production to assure proper function.
11. Ravenna also deploys environments locally using Vagrant to assure functionality before moving to staging or production.
12. All formal change requests require unique ID and authentication.
13. Ravenna uses the [Security Technical Implementation Guides (STIGs)](http://iase.disa.mil/stigs/) published by the Defense Information Systems Agency as a baseline for hardening systems.
...
3. Critical infrastructure services such as logging, monitoring, LDAP servers, or Windows Domain Controllers must be configured with appropriate Salt states.
   * These Salt states have been approved by the VP Engineering and CTO to be in accordance with all Ravenna policies, including setting appropriate:
...
1. Ravenna uses automated tooling to ensure systems are up-to-date with the latest security patches.
